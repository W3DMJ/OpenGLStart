...
Shader::Shader(const std::string& fileName)
{
    ...
    mShaders[0] = CreateShader(LoadShader(fileName + ".fs"), GL_FRAGMENT_SHADER);

    for (uint16_t i = 0; i < NUM_SHADERS; i++ )
...
GLuint Shader::CreateShader(const std::string& text, GLenum shaderType)
{
    ...
    glShaderSource(shader, 1, &shaderSourceStrings[0], &shaderSourceStringLength[0]);
